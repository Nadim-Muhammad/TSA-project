---
title: "TSA project - Forecasting financial instrument prices with VECM and ARIMA models"
...
```{r}
setwd("C:\\Users\\micha\\OneDrive\\Dokumenty\\QF\\GitHub\\TSA-project")
...
```{r warning=FALSE}
testdf(variable = chosen_ones$diff_first, max.augmentations = 3)
```
...
The cointegrating vector is: 1, -152.195497, 0.271123 which defines the cointegrating relationship as:\
...
The *long run* relationship is described by the parameter 0.271123 from the cointegrating relationship: so if `first` increases by 1 in the LONG RUN `second` will increase by 0.271123.
...
### Select VAR model with proper lag lenght
...
- 0-3 shows initial shock in impact with downward trend until near impact level of 0.

- 3-10 shows variation in impact with following movements: 3-5 upward, 5-6 downward, 6-7 upward, 7-9 downward, 9-10 upward.

- The impact in first is fairly stable from period 10 and shows little variation from 0,5. 

**from second:**

- Shock in impact with downward trend form 2 up to 10 period, since then the impact stays relatively stable with a low upward trend at around -1,5.

**Second:**

**from first:**

- The impact in first is fairly stable and showing little variation from 0 until period 16. 

**from second:**

- 0-12 initial shock in impact for second up to 12 period, since then the impact stays stable at around 0,05.
...
- Whole variation is from the first time series.

- the variation seems to be constant for first and for second.

- Overall the first shocks have much stronger impact on future values of first and second.

**Second:**

- In the initial periods from 1-3 whole variation is explained by second time series.

- The contribution of first to the variation for second changes fairly rapidly from 4-14.

- Afterwards the contribution of first to variation levels off at around 95%.

- Overall except initial state from 1-7 the first shocks have much stronger impact on future values of first and second.
...
The results are pretty similar to the earlier `VAR(6)` model with only difference being:

**for first:**

- In second there is a difference where from 24 period the results are completely stable at impact level around -1,5

**for second:**

- Slightly decreased level of impact in first making it  below zero 

- Higher result in second leaving it slightly above 0,05.
...
We chose the third and ninth time series to model and forecast using ARIMA and VECM. Both of them are integrated of order 1 and we found their cointegrated vector. When analyzing ACF and PACF graphs to chose proper orders of q and p for ARIMA models, we concluded that in those time series there are no seasonal patterns, which was later confirmed when by statistically insignificance in seasonal variables in VAR models (there were some statistical evidence of seasonality, but only for single seasonal variables and in one time series mostly, they were also near the significance line (p-value of 0.05)).

For ARIMA models we followed Box-Jenkins procedure, but we also started by creating ARIMA(1,1,1) model for both time series in order to set up some benchmark. When following Box-Jenkins procedure, we tried to have low orders of p and q (so the model will be more theoretical), but when ACF and PACF showed some correlations, we also check higher orders of p and q (which made the models more atheoretical).

For VECM, firstly we checked for Granger causality, from which we could conclude that both time series influences (causes) each other. Secondly, we checked information criterias in order to select the proper lag lenght of the VAR model, which will be used to create VECM model. We also plotted Impulse Response Function (IRF) and Forecast Error Variance Decomposition (FEVD).

After selecting models with the smallest values of Information Criterias, we forecasted 30 next periods and plotted them with they upper and lower bands (95%), and calculated ex-post errors (MAE, MSE, MAPE, AMAPE). For the third time series the ARIMA(6,1,4) turn out to be the best (smallest ex-post errors), and for ninth time series, the ARIMA(15,1,3) turned out to be the best (smallest ex-post errors). For those time series it turns out that ARIMA models are performing better than VECM.
